...
oReq.addEventListener("load", function () {
  ...
  function runTest(i) {
    if (i < tests.length) {
      let test = tests[i];
      if (test.enabled) {
        test.statement(test.name).then(function (serverResponse) {
  ...
  function showRegistrationScreen(withClearCredentialsDisabled = true) {
    $("label[for='label-content']").empty();
    $("button[id='register-button']").show();
    $("button[id='confirm-button']").hide();
    $("button[id='run-button']").hide();
    $("#register-input").val("").show();
    $("#pin-input").val("").hide();
  ...
  function showReadyScreen(withClearCredentialsDisabled = false) {
    $("label[for='label-content']").empty();
    $("button[id='register-button']").hide();
    $("button[id='confirm-button']").hide();
    $("button[id='run-button']").show();
    $("#register-input").val("").hide();
    $("#pin-input").val("").hide();
    $("#clear-credentials-button")
      .prop("disabled", withClearCredentialsDisabled)
      .show();
  ...
  function showInitialTestAgenda() {
    ...
    tests.forEach(function (test) {
      if (test.enabled) {
        $("label[for='agenda-items']").append(`<div style="margin: 0;">
                                 <input type="checkbox" id="cb-${mkId(
                                   test.name
                                 )}" name="use-cached-credentials" disabled/>
                                 <span style="color: #0082FF" id="${mkId(
                                   test.name
                                 )}"> ${test.name}</span>
                              </div>`);
      } else {
        $("label[for='agenda-items']").append(`<div style="margin: 0;">
                                 ...
                                 <span style="color: rgb(193 206 194)" id="${mkId(
                                   test.name
                                 )}"> ${test.name}</span>
                              </div>`);
      }
        ...
        switch (serverResponse.status) {
          case ServerResponse.OK: {
            $("label[for='label-content']")
              .empty()
              .html(
                "Please check your email for the confirmation PIN we sent you. <br> Enter it below then press <span style='color: #0082FF'>Confirm</span> to continue.<br />"
              )
              .css({
                "font-style": "normal",
                "color": "#000",
            ...
            $("button[id='register-button']").hide();
            $("button[id='run-button']").hide();
            $("button[id='confirm-button']").show();

            $("#register-input").val("").hide();
            $("#clear-credentials-container").hide();
            $("#pin-input").val("").show();
      ...
      .then((serverResponse) => {
        switch (serverResponse.status) {
          case ServerResponse.OK: {
            ...
            $("label[for='label-content']").remove();
  ...
  $(
    ...
    switch (clickEvent.currentTarget.id) {
      ...
      case "confirm-button": {
        ...
        doConfirm(pin).then((serverResponse) => {
          switch (serverResponse.status) {
            case ServerResponse.OK: {
              break;
            }
